...
gnome-terminal --title="Elasticsearch" -- elasticsearch-7.10.0/bin/elasticsearch
sleep 20

# Start kibana in a seperate terminal window.
gnome-terminal --title="Kibana" -- kibana-7.10.0-linux-x86_64/bin/kibana
sleep 10

# Remove olde sincedb files before launching logstash.
rm logstash-7.10.0/data/plugins/inputs/file/.sincedb_*
...
gnome-terminal --title="Logstash Kube-Hunter" -- logstash-7.10.0/bin/logstash -f LogstashConfigurationFiles/kubehunter.conf
sleep 30
...
gnome-terminal --title="Logstash Quay" -- logstash-7.10.0/bin/logstash -f LogstashConfigurationFiles/quay.conf
sleep 30
